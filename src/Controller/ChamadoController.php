...
class ChamadoController
{
    ...
    {
        if (empty($vo->getDescrciaoProblema()) || empty($vo->getNumero_nf()) || empty($vo->getCliente_id()) || empty($vo->getDefeito()) || empty($vo->getObservacao()) || empty($vo->getLoteID())) {
            return 0;
        }
        ...
        $vo->setIdLogado($vo->getTecnico_atendimento());
...
/*     public function GravarDadosOsController(ReferenciaOS $vo)
    ...
    public function RetornarEquipamentosLoteController($LoteID, $empresa_id)
    {
        
        return $this->dao->RetornarEquipamentosLoteDAO($LoteID, $empresa_id);
    }

    public function RetornarEquipamentosLoteControllerAdmin(ChamadoVO $vo):array
    {
        return $this->dao->RetornarEquipamentosLoteAdminDAO($vo); 
    }
