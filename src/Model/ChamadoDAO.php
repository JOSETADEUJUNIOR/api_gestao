...
class ChamadoDAO extends Conexao
{
    ...
    public function RetornarEquipamentosLoteDAO($LoteID, $empresa_id)
    {
        $sql = $this->conexao->prepare(ChamadoSQL::EquipamentosLoteSQL());

        $sql->bindValue(1, $LoteID);
    ...
    public function RetornarEquipamentosLoteAdminDAO(ChamadoVO $vo):array
    {
        $sql = $this->conexao->prepare(ChamadoSQL::EquipamentosLoteSQL());

        $sql->bindValue(1, $vo->getLoteID());
